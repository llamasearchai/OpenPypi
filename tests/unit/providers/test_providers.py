...
def test_provider_registration():
    ...
    assert "ai" in registry.list_ai_providers()
    
    # Test provider initialization
    github = registry.get_provider("github")({"token": "test"})
...
class TestProviderSecurity:
    ...
    def test_docker_image_quarantine(self):
        with patch.object(DockerProvider, 'build_image', return_value={
            "image_id": "test", "tags": ["vulnerable"], "success": True
        }):
            with patch.object(DockerProvider, 'security_scan_image', 
    ...
    def test_docker_high_concurrency(self):
        with ThreadPoolExecutor(50) as executor:
            futures = [executor.submit(
                DockerProvider().build_image, "Dockerfile"
            ) for _ in range(100)]
