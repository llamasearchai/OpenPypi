...
from openpypi.providers.base import AIBaseProvider, BaseProvider
...
class TestProviderSecurity:
    ...
    def test_docker_security_isolation(self):
        ...
        with patch("docker.DockerClient") as mock_docker:
            ...
            provider = DockerProvider({"socket_url": "unix:///var/run/docker.sock", "timeout": 10})
            ...
            try:
                with patch("requests.get") as mock_get:
                    ...
                    provider = registry.get_provider(
                        "github", {"token": f"test_{worker_id}" + "x" * 28}
                    )
    ...
    def test_github_docker_integration(self):
        ...
        with patch("requests.get") as mock_github:
            ...
            with patch("docker.DockerClient") as mock_docker:
                ...
                docker = registry.get_provider(
                    "docker", {"socket_url": "unix:///var/run/docker.sock"}
                )
...
class TestProviderCompliance:
    ...
    def test_ai_provider_compliance(self):
        ...
        # Get AI providers
        ai_providers = [
            name for name, cls in registry._providers.items() if issubclass(cls, AIBaseProvider)
        ]
