...
async def create_package(
    ...
    package = Package(
        name=project.name, version=request.version, project_id=project.id, status="pending"
    )
    ...
    background_tasks.add_task(build_package_async, package.id, project, request.build_options)
...
async def list_packages(
    ...
    return PackageListResponse(
        packages=package_responses, total=total, page=page, per_page=per_page
    )


@router.get("/{package_id}", response_model=PackageResponse)
async def get_package(
    package_id: str, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)
):
    """Get detailed information about a specific package."""
    package = (
        db.query(Package)
        .join(Project)
        .filter(Package.id == package_id, Project.owner_id == current_user.id)
        .first()
    )
...
async def rebuild_package(
    ...
    background_tasks.add_task(build_package_async, package.id, package.project, build_options)
...
async def download_package(
    ...
    if package.status != "success":
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Package is not ready for download"
        )
