...
async def list_all_projects(
    ...
    for project in projects:
        ...
        project_responses.append(ProjectAdminResponse(
            id=project.id,
            name=project.name,
...
async def get_detailed_health(db: Session = Depends(get_db)):
    ...
    recent_errors = db.query(AuditLog).filter(
        and_(AuditLog.created_at >= recent_time, AuditLog.success == False)
    ).count()
    ...
    recent_failed_builds = db.query(Package).filter(
        and_(Package.created_at >= recent_time, Package.status == "failed")
    ).count()
    
    build_success_rate = ((recent_builds - recent_failed_builds) / recent_builds * 100) if recent_builds > 0 else 100
