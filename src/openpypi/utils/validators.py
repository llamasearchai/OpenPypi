                ...
                if isinstance(node, ast.Import):
                    ...
                    if node.module and node.level == 0:
                        ...
                        if node.module in ['os', 'sys'] and any(
                            alias.name in ['system', 'exec', 'eval'] 
                            for alias in node.names
                        ):
                ...
                if isinstance(node, (ast.FunctionDef, ast.ClassDef, ast.AsyncFunctionDef)):
                    if not ast.get_docstring(node):
                        ...
                        if len(docstring.strip()) < 10:
                            issues.append(f"{type(node).__name__} '{node.name}' has very short docstring")
                ...
                if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                    ...
                    for arg in node.args.args:
                        if not arg.annotation and arg.arg != 'self' and arg.arg != 'cls':
                            issues.append(f"Argument '{arg.arg}' in function '{node.name}' missing type annotation")
...
class SecurityValidator:
    ...
    def validate_security(self, code: str) -> Dict[str, Any]:
        ...
        if not results['high_risk'] and not results['medium_risk']:
            results['recommendations'].append("Code appears secure, but consider additional security review")
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(mode='w', suffix='.txt', delete=False) as f:
                ...
                try:
                    ...
                    results['warnings'].append("Safety tool not installed - skipping vulnerability scan")
...
class TestValidator:
    ...
    def validate_tests(self, package_dir: Path) -> Dict[str, Any]:
            ...
            for test_file in test_dir.rglob('*.py'):
                if test_file.name.startswith('test_') or test_file.name.endswith('_test.py'):
                    ...
                    try:
                        ...
                        results['recommendations'].append(f"Failed to analyze {test_file}: {str(e)}")
        ...
        if not results['framework_detected']:
            results['recommendations'].append("Use a recognized test framework (pytest recommended)")
        ...
        try:
            ...
            if coverage_file.exists():
                ...
                for filename, file_data in files.items():
                    results['file_coverage'][filename] = file_data.get('summary', {}).get('percent_covered', 0)
                ...
                low_coverage_files = [
                    f for f, cov in results['file_coverage'].items() 
                    if cov < 70
                ]
...
class DocumentationValidator:
    ...
    def __init__(self):
        self.readme_sections = [
            'installation', 'usage', 'examples', 'api', 'contributing', 'license'
        ]
    ...
    def validate_readme(self, readme_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            results['quality_score'] = (section_score * 0.6 + length_score * 0.1 + 
                                      badge_score + example_score) * 100
            
            # Generate recommendations
            if results['length'] < 500:
                results['recommendations'].append("README is quite short - consider adding more detail")
...
class QualityValidator:
    ...
    async def validate_package_quality(
        ...
        logger.info(f"Quality validation completed. Overall score: {results['overall_score']:.1f} ({results['grade']})")
    ...
    async def _validate_all_code(self, package_dir: Path) -> Dict[str, Any]:
        ...
        for py_file in python_files:
            try:
                ...
                # Naming conventions
                naming_valid, naming_issues = self.code_validator.validate_naming_conventions(content)
                if not naming_valid:
                    results['naming_issues'].extend([f"{py_file}: {issue}" for issue in naming_issues])
                else:
                ...
                if not doc_valid:
                    results['docstring_issues'].extend([f"{py_file}: {issue}" for issue in doc_issues])
                else:
                ...
                if not type_valid:
                    results['type_hint_issues'].extend([f"{py_file}: {issue}" for issue in type_issues])
                else:
                    file_score += 20  # 20 points for type hints
                
                # Complexity
                complexity_valid, complexity_issues = self.code_validator.validate_complexity(content)
                if not complexity_valid:
                    results['complexity_issues'].extend([f"{py_file}: {issue}" for issue in complexity_issues])
                else:
    ...
    async def _validate_security(self, package_dir: Path) -> Dict[str, Any]:
        ...
        for py_file in python_files:
            try:
                ...
                all_issues = (security_results['high_risk'] + 
                ...
                results['code_security']['issues'].append(f"{py_file}: Security analysis failed - {str(e)}")
    ...
    def _generate_summary(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if results['code_quality']:
            ...
            summary['total_issues'] = (syntax_errors + naming_issues + doc_issues + 
                                     type_issues + complexity_issues)
    ...
    def _generate_comprehensive_recommendations(self, results: Dict[str, Any]) -> List[str]:
        ...
        if code_score < 0.7:
            recommendations.append("🔧 Improve code quality: add type hints, docstrings, and follow PEP 8")
        ...
        if results['overall_score'] < 60:
            recommendations.append("🎯 Focus on fixing critical issues first, then improve incrementally")
        elif results['overall_score'] < 80:
            recommendations.append("⭐ Good foundation! Polish remaining issues for production readiness")
        else:
            recommendations.append("🎉 Excellent quality! Consider advanced optimizations and features")
        
        return recommendations[:10]  # Limit to top 10 recommendations


# Utility functions for external use
async def validate_package_comprehensive(
    package_dir: Path,
    **kwargs
) -> Dict[str, Any]:
    """
