...
def publish_test() -> bool:
    ...
    result = run_command([
        ...
        "--username", "__token__",
        "--password", os.getenv("TEST_PYPI_TOKEN"),
        "dist/*"
    ], cwd=project_root, check=False)
...
def main():
    ...
    parser.add_argument("command", choices=[
        "check", "test", "lint", "security", "build", "test-publish", "publish", "full"
    ], help="Command to run")
    ...
    parser.add_argument("--force", action="store_true", help="Force publication even if checks fail")
